...
import {
  ...
  getPairPriceUSD,
  getPairFDV,
  getPairTokenTotalSupply,
  getPairInitialReserve
} from './pricing'
import {
  ...
  ZERO_BD,
  BI_18,
  createLiquiditySnapshot, isBuyTx, fetchTokenTotalSupply,
  ZERO_BI
} from './helpers'
...
export function handleSync(event: Sync): void {
  ...
  if (pair.initialReserve0.equals(ZERO_BD)) {
    pair.initialReserve0 = pair.reserve0
  }

  if (pair.initialReserve1.equals(ZERO_BD)) {
  ...
  if (bundle.maticPrice.notEqual(ZERO_BD)) {
    pair.reserveUSD = pair.reserveETH.times(bundle.maticPrice)
  }

  pair.FDV = getPairFDV(token0 as Token, token1 as Token, pair.priceUSD)
  pair.MKTCAP = getPairFDV(token0 as Token, token1 as Token, pair.priceUSD)

  pair.tokenTotalSupply = getPairTokenTotalSupply(token0 as Token, token1 as Token)
  pair.initialReserve = getPairInitialReserve(token0 as Token, token1 as Token, pair as Pair)

  // use tracked amounts globally
  uniswap.totalLiquidityETH = uniswap.totalLiquidityETH.plus(trackedLiquidityETH)
  if (bundle.maticPrice.notEqual(ZERO_BD)) {
    uniswap.totalLiquidityUSD = uniswap.totalLiquidityETH.times(bundle.maticPrice)
  }

  // now correctly set liquidity amounts for each token
  token0.totalLiquidity = token0.totalLiquidity.plus(pair.reserve0)
  token0.totalSupply = fetchTokenTotalSupply(pair.token0, token0.decimals)
  token1.totalLiquidity = token1.totalLiquidity.plus(pair.reserve1)
  token1.totalSupply = fetchTokenTotalSupply(pair.token1, token0.decimals)
...
export function handleSwap(event: Swap): void {
  ...
  let derivedAmountUSD = derivedAmountETH.times(bundle.maticPrice)
  ...
  if (bundle.maticPrice.equals(ZERO_BD)) {
    ...
    trackedAmountETH = trackedAmountUSD.div(bundle.maticPrice)
  }
  ...
  let isBuy = isBuyTx(token0 as Token, token1 as Token, amount0In, amount1In)
  ...
  // update six hourly pair data
  pairSixHourData.volumeToken0 = pairSixHourData.volumeToken0.plus(amount0Total)
  pairSixHourData.volumeToken1 = pairSixHourData.volumeToken1.plus(amount1Total)
  pairSixHourData.volumeUSD = pairSixHourData.volumeUSD.plus(trackedAmountUSD)
  ...
  token0DayData.dailyVolumeUSD = token0DayData.dailyVolumeUSD.plus(
    amount0Total.times(token0.derivedETH as BigDecimal).times(bundle.maticPrice)
  )
  token0DayData.save()
  ...
  token1DayData.dailyVolumeUSD = token1DayData.dailyVolumeUSD.plus(
    amount1Total.times(token1.derivedETH as BigDecimal).times(bundle.maticPrice)
  )
  token1DayData.save()
}