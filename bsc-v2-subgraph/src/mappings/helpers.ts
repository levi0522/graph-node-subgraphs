...
export function fetchTokenTotalSupply(tokenAddress: string, decimals: BigInt): BigDecimal {
  let contract = ERC20.bind(Address.fromString(tokenAddress))
  let totalSupplyResult = contract.try_totalSupply()
  if (!totalSupplyResult.reverted) {
    return convertTokenToDecimal(totalSupplyResult.value, decimals)
  }else{
    return ZERO_BD
  }
}
...
} else if (!STABLECOINS.includes(token0.id) && STABLECOINS.includes(token1.id)) {
